// admin-routes.js — Gestione carte usate (admin) — v2
...
module.exports = function(pool, ADMIN_LOGIN){
  ...
  router.get('/used-cards', async (req, res) => {
    try{
      ...
      // Mappa tipi ITA→EN e accetta entrambi
      const k = String(q.kind||'').toLowerCase();
      let kinds = null;
      if (k) {
        if (k === 'creature') kinds = ['creature','creatura'];
        else if (k === 'incantesimo' || k === 'spell') kinds = ['spell','incantesimo'];
        else if (k === 'istantanea' || k === 'instant') kinds = ['instant','istantanea'];
      }
      ...
      const params = [];
      if (kinds){ where.push(`i.kind = ANY($${params.length+1})`); params.push(kinds); }
